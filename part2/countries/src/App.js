...
const Show_weather_pro = ({ special_country }) => {
  ...
  const w_hook = () => {
    axios
      .get(
        `https://api.openweathermap.org/geo/1.0/direct?q=${special_country.capital}&limit=1&appid=${api_key}`
      )
      .then((res) => {
  ...
  useEffect(w_hook, [special_country]);
  console.log(weather_pro);
  const temperature = `${
    weather_pro && weather_pro.main && weather_pro.main.temp
      ? (weather_pro.main.temp - 273.15).toFixed(2)
      : "N/A"
  } Celcius`;
  const wea_picture =
    weather_pro && weather_pro.weather
      ? `https://openweathermap.org/img/wn/${weather_pro.weather[0].icon}.png`
      : null;

  const wind = `${
    weather_pro && weather_pro.wind && weather_pro.wind.speed
      ? weather_pro.wind.speed.toFixed(2)
      : "N/A"
  } m/s`;

  return (
    <div>
      <h1>weather in {special_country.capital}</h1>
...
const Show = ({ props, add_new_special_coutry }) => {
  ...
  if (props === null) {
    ...
    const list_languages = end_language.map((lan, id) => (
      <li key={id}> {lan}</li>
    ));
...
const Display = ({ content, add_new_special_coutry, search_content }) => {
  ...
  if (content.length >= 10 && search_content !== "") {
    ...
    return <div>Please enter the country name!</div>;
  } else {
    console.log("------", content);
    ...
    return (
      <div>
        {content.map((cou, id) => (
          <Extend_dis
            instance={cou}
            add_new_special_coutry={add_new_special_coutry}
            key={id}
          />
        ))}
      </div>
...
const App = () => {
  ...
  const content = country.filter((cou) =>
    cou.name.common.toLowerCase().includes(search_content.toLowerCase())
  );
